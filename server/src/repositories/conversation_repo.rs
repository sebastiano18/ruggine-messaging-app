...
"#
        ...
        for row in rows {
            ...
            conversations.push(ConversationWithLastMessage {
                ...
                last_msg_id: row
                    .get::<Option<String>, _>("last_msg_id")
                    .and_then(|s| Uuid::parse_str(&s).ok()),
                last_msg_author_id: row
                    .get::<Option<String>, _>("last_msg_author_id")
                    .and_then(|s| Uuid::parse_str(&s).ok()),
...
"#,
        ...
        for row in member_rows {
            ...
            let joined_at = row.get::<Option<i64>, _>("joined_at");
...
"#;
    ...
    pub async fn create_group(pool: &SqlitePool, name: &str, owner_id: Uuid) -> Result<Uuid> {
        ...
        let ts = chrono::Utc::now().timestamp(); // ← Salva il timestamp
        ...
        sqlx::query(
            ...
            .bind(name)
            .bind(&owner_id_str)
            .bind(ts) // ← Usa la variabile
            .execute(pool)
            .await?;

        sqlx::query(
            "INSERT INTO participants (conversation_id, user_id, role, joined_at) VALUES (?, ?, 'owner', ?)" // ← Aggiungi joined_at
        )
            .bind(&cid_str)
            .bind(&owner_id_str)
    ...
    pub async fn add_member(
        ...
        sqlx::query(
         ...
         VALUES (?, ?, 'member', ?, 0)"
            ...
            .bind(joined_at)  // ← Aggiungi questo
            .execute(pool)
            .await
    ...
    pub async fn user_has_dm_access(
        pool: &SqlitePool,
        conversation_id: Uuid,
        user_id: Uuid,
    ) -> Result<bool> {
    ...
    pub async fn list_participant_ids(
        ...
        let id_strs: Vec<String> =
            sqlx::query_scalar("SELECT user_id FROM participants WHERE conversation_id = ?")
                .bind(&conv_id_str)
                .fetch_all(pool)
                .await
                .map_err(AppError::from)?;
