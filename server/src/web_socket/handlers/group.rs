...
pub async fn handle_invite_user(
    ...
    for target_username in target_usernames {
        ...
        // Ottieni partecipanti esistenti (escluso il nuovo membro)
        let participant_ids = match ConversationService::list_participant_ids(
            &state.pool,
            conversation_id
        ).await {
            Ok(ids) => ids.into_iter().filter(|&id| id != target_user_id).collect::<Vec<_>>(),
            Err(e) => {
        ...
        if !participant_ids.is_empty() {
            ...
            if let Err(e) = send_event_to_multiple_users(
                ...
                warn!("Failed to send member_added events: {}", e);
            }
        }
...
pub async fn handle_remove_member(
    ...
    if !remaining_participants.is_empty() {
        ...
        if let Err(e) = send_event_to_multiple_users(
            ...
            warn!("Failed to notify remaining participants: {}", e);
        }
    }
